...
* **(r52, petición de Ricardo: «solución híbrida») La parte visual SÍ es `QDrag.exec()` real** (el fantasma semitransparente que sigue al cursor y el de «permitido/prohibido» los pinta Windows, con más fidelidad que cualquier cosa hecha a mano), **pero el destino no depende de que Qt dispare `dropEvent`.** `_ThumbList` guarda dónde se pulsó (`mousePressEvent`) y, en cuanto el ratón se mueve más de `QApplication.startDragDistance()` (`mouseMoveEvent`), arranca un `QDrag` con un fantasma (ver r81) y lo lanza con `exec()` — que bloquea, pero **siempre** devuelve el control al soltar el botón, se haya aceptado el drop o no. Al volver, `_run_drag` lee la posición real del cursor con `QCursor.pos()` (la da el sistema operativo, no depende de Qt) y hace lo mismo que antes de r52: `drop_row()` + `move_selection_to()`. `dropEvent` se ignora a propósito, sin fiarse de que llegue. Solo activo con `organizing=True`; fuera de ese modo el clic se comporta como siempre (selecciona y navega).
* **(r80, petición de Ricardo: «al arrastrar una miniatura, la parte inferior de la pantalla se rellena de negro») El fantasma se creaba con `QPixmap(rect.size())`, en píxeles lógicos, sin `devicePixelRatio`**: con el escalado de Windows (125 %, 150 %…, lo habitual en portátiles) el mapa de bits físico salía más pequeño de lo que el sistema esperaba, y la imagen de arrastre que compone el propio Windows (OLE, no Qt) quedaba mal dimensionada. Ajustado el tamaño físico, pero **el negro seguía saliendo** (r81): quedaba otra causa.
* **(r81, petición de Ricardo: «el problema persiste; si es la miniatura, cámbiala por un rectángulo en blanco») El fantasma ya no recorta la miniatura real: es un rectángulo liso** (blanco, borde azul) del mismo tamaño, construido con `QImage(size, Format_ARGB32_Premultiplied)` en vez de `QPixmap(size)` (formato ambiguo, típicamente sin alfa premultiplicado — la causa típica de que una imagen de arrastre de Windows, que compone él mismo por OLE, pinte en negro donde debería verse transparente). Probado con arrastres reales simulados (mueve el ratón de verdad, mantiene el botón, captura la pantalla —con el cursor, que las capturas normales no recogen— antes de soltar): el fantasma sale gris claro, no negro, y `test_arrastrar_una_miniatura_la_traslada_de_verdad` sigue en verde.
* **(r82) Ricardo lo seguía viendo negro en su pantalla real** aun con el arreglo de r81, pero ni una captura (mía, con cursor incluido) ni una grabación suya con la Herramienta Recortes lo recogían — es decir, el negro es real en la composición **en pantalla**, no en el mapa de bits que cualquier herramienta de captura lee (por eso nadie pudo verlo hasta ahora: ninguna herramienta de captura, ni siquiera grabando, pasa por esa capa). Consistente con un fallo de composición del **canal alfa** de la ventana en capas que usa Windows para esta imagen de arrastre — con el alfa premultiplicado de r81 puesto en duda igualmente. **Se quita el canal alfa del todo**: `Format_RGB32` en vez de `Format_ARGB32_Premultiplied`, relleno opaco (celeste claro `#EAF3FC`, borde azul `#0078D4`, sin `setOpacity`). Sin alfa que la composición pueda mezclar mal, no hay semitransparencia que falle — a cambio, el fantasma deja de verse a través. Comprobado (`HKCU\…\AppCompatFlags\Layers` vacío, `EnableTransparency` = 1): no es una capa de compatibilidad ni la transparencia del sistema desactivadas para el usuario. **Pendiente de que Ricardo confirme en su pantalla real** si el negro desaparece del todo sin alfa.
